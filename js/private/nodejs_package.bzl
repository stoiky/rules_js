...
""",
    ...
    "link_package_guard": attr.string(
        doc = """When explictly set, check that package of this target matches
        the value.
...
def _impl(ctx):
    if ctx.attr.link_package_guard != ".":
        if ctx.label.package != ctx.attr.link_package_guard:
            fail("expected this target to be in package %s, but it is in %s" % (ctx.attr.link_package_guard, ctx.label.package))
